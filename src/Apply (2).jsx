...
export default function Apply() {
  ...
  const validate = () => {
    ...
    if (form.hasEquipment === null) e.hasEquipment = "Please answer";
    setErrors(e);
    ...
    <div className="min-h-screen bg-[#F7F9FC] font-body flex items-center justify-center p-5">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 max-w-lg w-full overflow-hidden my-8">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <p className="text-xs font-mono text-slate-500 uppercase tracking-wide mb-2">Do you have your own equipment?</p>
            <div className="flex gap-2">
              {[{ label: "Yes", value: true }, { label: "No", value: false }].map((opt) => (
                <button
                  key={opt.label}
                  ...
                  className={`flex-1 px-3 py-2.5 rounded-xl text-sm font-semibold border transition-colors ${form.hasEquipment === opt.value ? "bg-[#0B5FFF] border-[#0B5FFF] text-white" : "bg-white border-slate-200 text-slate-500"}`}
                >
                  {opt.label}
                </button>
              ))}
            </div>
            {errors.hasEquipment && <p className="text-red-500 text-xs mt-1">{errors.hasEquipment}</p>}
          </div>
