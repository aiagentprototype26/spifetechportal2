...
const EMPTY_TECH_FORM = {
  firstName: "", lastName: "", phone: "", email: "", city: "", services: [], locations: [], hasVehicle: null, hasEquipment: null,
};
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wide">Has Equipment?</p>
              <p className="text-slate-700">{tech.hasEquipment === true ? "Yes" : tech.hasEquipment === false ? "No" : "—"}</p>
            </div>
          </div>
...
function AddTechnicianForm({ onAdd }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
      ...
      <div>
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Has Own Equipment?</p>
        <div className="flex gap-2">
          {[{ label: "Yes", value: true }, { label: "No", value: false }].map((opt) => (
            <button
              key={opt.label}
              type="button"
              onClick={() => setForm((f) => ({ ...f, hasEquipment: opt.value }))}
              className={`flex-1 px-3 py-2 rounded-xl text-sm font-semibold border transition-colors ${form.hasEquipment === opt.value ? "bg-[#0B5FFF] border-[#0B5FFF] text-white" : "bg-white border-slate-200 text-slate-500"}`}
            >
              {opt.label}
            </button>
          ))}
...
function DispatcherDashboard() {
  ...
  const handleAddTech = async (techForm) => {
    try {
      await addDoc(collection(db, "technicians"), { ...techForm, pin: genPin(), online: false });
  ...
  const handleApproveTech = async (techId) => {
    try {
      ...
      const pin = tech?.pin || genPin();
      await updateDoc(doc(db, "technicians", techId), { status: "active", pin, online: false });
      showToast(`${tech?.firstName || "Technician"} approved. PIN: ${pin}`);
    } catch (e) {
      showToast(e.message, "error");
    }
  };
...
export default function Dispatcher() {
  ...
  // Restore by uncommenting the block below and removing this early return.
